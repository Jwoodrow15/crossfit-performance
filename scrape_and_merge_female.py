...
logger.setLevel(logging.DEBUG)  # Changed to DEBUG to capture all logs

# File handler for detailed logs (DEBUG and above) - specific to female scraping
file_handler = logging.FileHandler("scrape_female.log")
...
# Batch configuration
BATCH_SIZE = 500          # Number of athletes per batch
TOTAL_LIMIT = 150000         # Total number of athletes to process in this run
...
async def scrape_athletes_with_missing_benchmarks(athlete_ids):
    semaphore = asyncio.Semaphore(20)  # Increased concurrency for larger batches
...
def scrape_and_merge_with_csv(input_file, output_file, batch_size=250, total_limit=10000, delay_min=15, delay_max=30):
    ...
    if os.path.exists(output_file):
        athlete_data = pd.read_csv(output_file, low_memory=False, dtype=str)
        logger.info(f"Loaded existing data from '{output_file}'")
    else:
        athlete_data = pd.read_csv(input_file, low_memory=False, dtype=str)
    ...
    for batch_num in range(num_batches):
        ...
        # Display batch information in the terminal
        ...
        # Log the counts next to each other at the end of the batch
        print(f"Back Squat count before scraping: {back_squat_before} | Back Squat count after scraping: {back_squat_after}")
...
if __name__ == "__main__":
    ...
    input_csv = "crossfit_open_2024_female_data.csv"
    output_csv = "crossfit_open_2024_female_with_benchmarks.csv"
